...
from AliceGit.Exceptions import AlreadyGitRepository, DirtyRepository, InvalidUrl, NotGitRepository, PathNotFoundException
...
class AliceGit:
	...
	@classmethod
	def clone(cls, url: str, directory: Union[str, Path], branch: str = 'master', makeDir: bool = False, force: bool = False, quiet: bool = True) -> AliceGit:
		if isinstance(directory, str):
			directory = Path(directory)
		...
		if quiet:
			cmd = f'{cmd} --quiet'
		subprocess.run(cmd)
		return AliceGit(directory=directory, url=url, quiet=quiet)
